"""
MAIN FNO DRIVER (Standard Benchmark)
------------------------------------
Function:
    The primary entry point for training the standard Fourier Neural Operator (FNO)
    on the NACA dataset using the neuraloperator library's Trainer API.

Key Features:
    1. Standard Implementation: Configured with the baseline hyperparameters (Modes=12, Width=32)
       used to establish the "Gold Standard" performance for this project.
    2. Integrated Logging: Extends the library's Trainer class to export detailed
       training metrics (Loss, Learning Rate, Time) to Excel for analysis.
    3. Automated Visualization: Generates the standard 6-panel comparison plot (Full View + Zoom)
       immediately after training to verify physical accuracy.
"""
...
EXCEL_FILENAME = os.path.join(MODEL_PATH, "naca_training_log.xlsx")  # --- MODIFIED ---
...
class VerboseTrainer(Trainer):
    def __init__(self, model, device, n_epochs, optimizer):
        ...
        self.history = []  # --- MODIFIED: Initialize history list ---
    ...
    def train(self, train_loader, test_loaders=None, training_loss=None, eval_losses=None, scheduler=None):
        for epoch in range(self.n_epochs):
            ...
            # --- MODIFIED: Get LR at start of epoch ---
            if scheduler:
                # get_last_lr() returns a list, get the first element
            ...
            # --- MODIFIED: Store metrics in history ---
...
with torch.no_grad():
    ...
    # --- MODIFIED: Cleaned up plotting calls ---
    ...
    # plt.show() # Disabling automatic show to prevent blocking in some environments