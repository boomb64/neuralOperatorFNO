"""
STANDARD FNO BENCHMARK
----------------------
Function:
    Trains a standard Fourier Neural Operator (FNO) on the NACA airfoil dataset using the
    neuraloperator library's Trainer API, but with custom logging modifications.

Key Features:
    1. Baseline Performance: Serves as the "Control Group" for all other experiments (Upscaling,
       Full Physics, etc.), establishing the expected accuracy on standard resolution data.
    2. Logging Upgrade: Modified to export detailed epoch-by-epoch training history (Loss, Time, LR)
       to Excel, enabling easy post-hoc analysis.
    3. Metric Parity: Uses the same LpLoss metric as other scripts to ensure apple-to-apple comparisons.
"""
